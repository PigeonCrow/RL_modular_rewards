# %% imports
import matplotlib.pyplot as plt
import numpy as np


# %% grid world room environment
class RoomEnv:  # room with 1 agent, all square are 0 and agent is 1 #(BaseEnv):
    def __init__(
        self,
        room_size=6,
        start_position=[0, 0],
        reward_position=None,
    ):
        self.room_size = room_size
        print(f"Initilised room of size: {room_size}x{room_size}")
        if start_position is None:
            # self.start_point = [room_size - 1, room_size - 1]
            self.start_point = [
                np.random.randint(0, int(room_size / 2)),
                np.random.randint(0, int(room_size / 2)),
            ]  # reward the agent at top half of grid[x,y]
            print(f"Start is at {self.start_point}")
        else:
            self.start_point = start_position
        self.agent_position = self.start_point
        if reward_position is None:
            # self.reward_position = [room_size - 1, room_size - 1]
            self.reward_position = [
                np.random.randint(int(room_size / 2), self.room_size),
                np.random.randint(int(room_size / 2), room_size),
            ]  # reward the agent at second half of grid[x,y]
        else:
            self.reward_position = reward_position
        print(f"Reward is at {self.reward_position}")
        self.state = np.zeros((room_size, room_size))
        self.state[self.start_point[0], self.start_point[1]] = 1  # position the agent at start_position[x,y]
        self.state[self.reward_position[0], self.reward_position[1]] = -1
        self.done = False
        self.rewards = 0  # Received from rewards

    def action_space(self):
        return [0, 1, 2, 3]

    def step(self, a=None, simulated=False):
        agent_position_x, agent_position_y = self.agent_position
        room_size = self.room_size
        if a != 0 and a != 1 and a != 2 and a != 3:
            raise ValueError("Action must be 0,1,2,3")  # catch if there is a bug here
        if a == 0:  # Move up
            if not agent_position_y >= room_size - 1:  # Check if agent can move up
                agent_position_y += 1
        elif a == 1:  # Move right
            if not agent_position_x >= room_size - 1:  # Check if agent can move right
                agent_position_x += 1
        elif a == 2:  # Move Down
            if not agent_position_y <= 0:  # Check if agent can move down
                agent_position_y -= 1
        elif a == 3:  # Move Left
            if not agent_position_x <= 0:  # Check if agent can move left
                agent_position_x -= 1
        if not simulated:
            self.state[self.agent_position] = 0
            self.agent_position = [agent_position_x, agent_position_y]
            self.state[self.agent_position] = 1  # update the agents poistion on the state map
        return [agent_position_x, agent_position_y]

    def terminate(self, pos):
        if self.state[pos[0], pos[1]] != -1:
            self.done = True
        return True

    def reset(self):
        self.rewards = 0  # reset rewards to 0
        self.state[self.start_point[0], self.start_point[1]] = 1
        self.agent_position = self.start_point
        self.state[self.reward_position[0], self.reward_position[1]] = -1

    def visualize_env(self):
        plt.grid(True, alpha=0.2)
        plt.imshow(self.state)
        plt.xticks(range(self.room_size))
        plt.yticks(range(self.room_size))
        plt.tight_layout()
        plt.show()  # plot the state of the

    def find_idx(self, pos):
        x, y = pos
        return (x * self.room_size) + y

    def get_pos(self, pos):
        x, y = divmod(pos, self.room_size)
        return [x, y]

    def state_dim(self):
        return self.room_size**2


# %% main
if __name__ == "__main__":
    print("Creating an environment:")
    room_env = RoomEnv(room_size=10, start_position=None)
    room_env.visualize_env()

# %%
